...
"""
class CCImageToCellAndTransitionLabel(IAugmentationFunction):
    def __call__(self, cc_image):
        transitions = ExtractCellTransitionsFromCCImage()(cc_image)
        class_image = ConnectedComponentImageToBinaryImage()(cc_image)
        class_image[transitions == 1] = CELL_EDGE_IDX
        return class_image


class CCImageToCellAndEdgeLabel(IAugmentationFunction):
    def __call__(self, cc_image):
        edges = ExtractEdgesFromCCImage()(cc_image)
        class_image = ConnectedComponentImageToBinaryImage()(cc_image)
        class_image[edges == 1] = EDGE_IDX
        return class_image
"""
...
class ExtractCellTransitionsFromCCImage(IAugmentationFunction):
    def __init__(self, kernel_size):
        self.kernel_size = kernel_size

    def __call__(self, cc_image):
        """Extract the pixels of adjacent cells.

        Parameters
        ----------
        cc_image : connected_component image
        0 is background, an integer means pixel belongs to cell i.
        Problems if number of cells > 255!
        kernel_size : int, optional
        border is computed via dilation. The input determines the dilations size
        (the default is config.get('edge_kernel_size')).

        Returns
        -------
        boolean image
        which of the image pixels belong to the transition class.
        """
        if cc_image.dtype != 'uint8':
            cc_image = cc_image.astype('uint8')

        kernel = np.ones((self.kernel_size, self.kernel_size))

        dilated_difference = cv2.dilate(cc_image, kernel).astype(
            'float32') - cc_image.astype('float32')
        cell_to_cell_transition = (dilated_difference > 0) * (cc_image > 0)

        return cell_to_cell_transition


class ExtractEdgesFromCCImage(IAugmentationFunction):

    def __init__(self, kernel_size):
        self.kernel_size = kernel_size

    def __call__(self, cc_image,
                 edge_is_part_of_cell=False
                 ):
        """Extract edges. Either object - background or object - other object.
        ...
        kernel_size : int, optional
            edges are extracted via dilation or erosion, the value defines
            the kernel size of the operator (the default is 
            config.get('edge_kernel_size')).
        edge_is_part_of_cell : bool, optional
            If true former cell pixels are marked as an edge; else,
            background pixels are used (the default is False).

        Returns
        -------
        binary image: np.array of same size and dtype boolean
            edge pixels are marked as true (or > 0).
        """
        if cc_image.dtype != 'uint8':
            cc_image = cc_image.astype('uint8')

        kernel = np.ones((self.kernel_size, self.kernel_size))
        if edge_is_part_of_cell:
            edges = ((cc_image - cv2.erode(cc_image, kernel))
                     > 0).astype('float32')
        else:
            # edge is part of background
            edges = ((cv2.dilate(cc_image, kernel).astype(
                'float32') - cc_image.astype('float32')) > 0)
